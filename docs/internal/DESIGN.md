...
### UI tier ladder
TechnoDrone UI sorts into three tiers. Pick the lightest tier that does the job. Engineered indicators are not a license for chrome.

- **Floating typography** — labels, values, score, kills, stage. Default for all text.
- **Engineered indicators** — slanted cells, bars, arcs, hairline brackets, thin-stroke containers. Glow-driven geometry that encodes real state. Allowed in the HUD; this is the language LIVES, BASS DROP, FLOW STATE, the stage timer, the heat arc, and the play-area corners already speak.
- **Containing boxes** — pause, settings, modals, hero confirmation moments. Thin strokes, low-alpha fills, never opaque slabs.

### Rules
- Do not add decorative lines, tails, accent rails, dividers, or underlines inside the playfield unless they communicate real gameplay information.
- The playfield interior stays minimal floating typography (Bucket 1: Resogun / Geometry Wars / Hyper Demon). Stage timer at the top, peripheral indicators near the ship, no chrome inside the play area.
- The right-side HUD is a designed CRT terminal panel and is the exception to the playfield's minimalism rule. It may use bezels, sub-display cells, hairline cell borders, scanlines, corner brackets, and instrument-cluster dividers. The asymmetry is intentional — the playfield is the action, the sidebar is the cab.
- Sidebar chrome must still earn its space: every line, bracket, or cell encodes structure (which stat lives where) or sells the CRT metaphor. No purely decorative ornament.
...
## Box Treatment Language
When a box is the right tier, it inherits this motif. These rules apply to engineered indicators (cells, bars, brackets) and to containing boxes (pause, settings, modals).

- **Corner geometry**: 8px cut corners, matching the existing LIVES/BASS DROP cells. No rounded pills.
- **Strokes**: 1–2px, single border. No double borders, no inset/outset.
- **Fills**: alpha `0.10`–`0.25` max. Solid slabs are forbidden outside full-screen modal overlays.
- **Glow**: on the stroke, not the fill. `shadowBlur` 8–16. Single pass.
- **Color sourcing**: pull from the Core Palette. Boxes do not introduce new accent colors.
- **Decoration**: no inner rules, no header bars, no tab tongues, no decorative cut-marks. The box is a frame, not a graphic.

### Yes
- Slanted cells with cut corners (LIVES, BASS DROP).
- Hairline corner brackets framing the playfield.
- Thin-stroke containers around pause and settings.
- Gradient selection chips with cut corners on title/death/win.

### No
- Opaque slabs.
- Drop-shadowed cards.
- Rounded pills.
- Decorative rules under floating labels.
- Double-stroke borders.
- Header strips on top of panels.
...
- Floating typography for values; engineered indicators for state (cells, bars, arcs, brackets)
